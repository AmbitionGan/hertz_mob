    ...
    <div class="findCar" :class="{'loaded': $store.state.isCalendarLoaded}">
        ...
        <div class="landMark clear">
            ...
            <div class="takeCarland right">
                ...
                <span
                    class="ellipsis"
                    @click="goLandMark('take')"
                >{{$store.state.takeCarLand === "" ? '请输入取车地点' : $store.state.takeCarLand }}</span>
            </div>
            ...
            <div class="retCarland right">
                ...
                <span
                    class="ellipsis"
                    @click="goLandMark('ret')"
                >{{$store.state.retCarLand === "" ? '请输入还车地点' : $store.state.retCarLand }}</span>
            </div>
        </div>
        <!-- 日历选择 -->
        <div class="calendar clear">
            <div class="takeCarDate left clear" @click="choiceDate('take')">
                ...
                <div class="right clear">
                    ...
                    <p
                        class="time"
                    >{{$store.state.takeCarHour + ':' + $store.state.takeCarMin + ' ' + $store.state.takeCarPeriod}}</p>
                </div>
            </div>
            ...
            <div class="retCarDate right" @click="choiceDate('ret')">
                ...
                <div class="right clear">
                    ...
                    <p
                        class="time"
                    >{{$store.state.retCarHour + ':' + $store.state.retCarMin + ' ' + $store.state.retCarPeriod}}</p>
                </div>
            </div>
        </div>
        <!-- 搜索按钮 -->
            ...
            >
                <div class="changeData">
                    <span
                        ...
                        :class="{'disable': isDisablePrevBtn}"
                    ></span>
                ...
                <div class="dayList">
                    ...
                    <ul class="clear">
                        <li
                            ...
                            :key="index"
                            :class="{'disable': isSwitchDate(item.daystring, 'disable'), 'over': isSwitchDate(item.daystring, 'over'), 'takeCar': isSwitchDate(item.daystring, 'takeCar'), 'retCar': isSwitchDate(item.daystring, 'retCar'), 'nonMonth': isSwitchDate(item.daystring, 'nonMonth'), 'today': item.istaday}"
                ...
                <div class="loadingBox" @click="() => {return false}">
                    <div>
        ...
        <div
            class="timeLayer"
            v-show="isShowTimeLayer"
            @click="isShowTimeLayer = false; if (!isShowTakeCarClr) {getTimeSpans()}"
        >
            <div class="timeLayerBox" @click="$event.stopPropagation()">
                <div class="timeShow">
                    <div class="center">
                        <span
                            class="hover left"
                            :class="{'active': isChooseHour}"
                            @click="chooseHourOrMin('hour')"
                        >{{temporaryStorageTime[0]}}</span>
                        <i class="left">:</i>
                        <span
                            class="min left"
                            :class="{'active': !isChooseHour}"
                            @click="chooseHourOrMin('min')"
                        >{{temporaryStorageTime[1]}}</span>
                    </div>
                    <div class="chooseIsAm">
                        <span
                            :class="{'active': temporaryStorageTime[2] === 'AM'}"
                            @click="choosePeriod('AM')"
                        >AM</span>
                        <span
                            :class="{'active': temporaryStorageTime[2] === 'PM'}"
                            @click="choosePeriod('PM')"
                        >PM</span>
                    </div>
                </div>
                <div class="timeChoose">
                    <div class="timeDisc">
                        <ul>
                            <li
                                v-for="(item, index) in timeLayerShowData"
                                :key="index"
                                :class="{'active': index == timeActiveIndex}"
                            >{{item}}</li>
                        </ul>
                        <div class="pointer">
                            <span></span>
    ...
    methods: {
        ...
        isSwitchDate(dayString, style) {
            ...
            switch (style) {
                ...
                case "nonMonth":
                    this.calendarLayerMonth !=
                    pubMethod.formatDateString(dayString)[1]
                        ? (returnBoolean = true)
                        : (returnBoolean = false);
                    break;
                default:
        ...
        choiceDetailDate(event, index) {
            ...
            // 当前点击的日期字符串
            let formatDateString = pubMethod.formatDateString(
                curClickDate.daystring
            );
            ...
            if (this.isChooseHour) {
                ...
                if (this.temporaryStorageTime[0] > 6) {
                    movePointer.style.transform =
                        "rotate(" +
                        (this.temporaryStorageTime[0] - 12) * 30 +
                        "deg)";
                } else {
                    movePointer.style.transform =
                ...
                if (this.temporaryStorageTime[1] / 5 > 6) {
                    movePointer.style.transform =
                        "rotate(" +
                        (this.temporaryStorageTime[1] / 5 - 12) * 30 +
                        "deg)";
                } else {
                    movePointer.style.transform =
                        "rotate(" +
                        (this.temporaryStorageTime[1] / 5) * 30 +
                        "deg)";
                }
        ...
        moveTouch(
            ...
            let moveTouch =
                style === "end" ? e.changedTouches[0] : e.touches[0];
            ...
            movePointer.style.transform =
                "rotate(" +
                this.pointConversion(moveX - originX, originY - moveY).toFixed(
                    0
                ) +
                "deg)";
            ...
            if (this.isChooseHour) {
                ...
                this.$set(
                    this.temporaryStorageTime,
                    1,
                    this.minData[realIndex]
                );
            }
            if (style === "end") {
                movePointer.style.transform =
                    "rotate(" + stopDegIndex * 30 + "deg)";
            }
        },
         ...
         */
        goLandMark(style) {
            this.$router.push({path: '/choiceCity', query:{'isTakeCar': style === "take" ? 1 : 0}})
        },
        ...
        goList() {
            if (this.$store.state.takeCarLand == "") {
                this.$Toast.show("请输入取车地点");
                return;
            }
            if (this.$store.state.retCarLand == "") {
                this.$Toast.show("请输入还车地点");
                return;
            }
            ...
            // 取还车品牌
            let brand =
                this.$store.state.takeCarBrand +
                "," +
                this.$store.state.retCarBrand;
            // 取还车guid
            let guid =
                this.$store.state.takeCarGuid +
                "," +
                this.$store.state.retCarGuid;
            // 取车信息
            let take = encodeURI(this.$store.state.takeCarLand);
            // 还车信息
            let ret = encodeURI(this.$store.state.retCarLand);
            // 租期
